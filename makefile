...
index.%: $(chapters) chapters/media/phototropism.gif
	pandoc $(chapters) --sandbox=false --extract-media=./media --request-header="User-Agent:pandoc/1.0 (jncraton@gmail.com)" --embed-resources --bibliography ref.bib --csl ieee.csl -f markdown+citations --lua-filter wp.lua --standalone --citeproc --toc -M fig-prog=true --variable=space_before_figure=0 -o $@
