all: index.html index.epub chunkedhtml

chapters = $(wildcard chapters/*.md)

pandocargs = --css style.css --extract-media=./media --request-header="User-Agent:pandoc/1.0 (jncraton@gmail.com)" --bibliography ref.bib --csl ieee.csl -f markdown+citations --lua-filter wp.lua --standalone --citeproc --toc

index.%: $(chapters) style.css chapters/media/phototropism.gif
	pandoc $(chapters) $(pandocargs) -o $@

chunkedhtml: $(chapters) style.css chapters/media/phototropism.gif
	pandoc $(chapters) $(pandocargs) -t chunkedhtml -o $@
	cp style.css chunkedhtml/style.css
